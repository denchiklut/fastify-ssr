...
import { getENV, setEnvVars, publicPath } from 'src/common'
...
export const getStats = (res: ServerResponse): ChunkExtractorOptions => {
	if (IS_PROD)
		return { statsFile: resolve(__dirname, '../client/loadable-stats.json') }
...
export const getApp = (
	...
	const statsCompilation = stats?.children?.find(
		child => child.name === 'server'
	)
